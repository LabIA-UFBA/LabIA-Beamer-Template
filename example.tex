...
\begin{frame}
    ...
    \begin{proof}
        \begin{enumerate}
            ...
            \item<2-> Let $q$ be the product of the first $p$ numbers.\footnotemark[1]
            \item<3-> Then $q+1$ is not divisible by any of them.\footnotemark[1]
            \item<1-> Thus $q+1$ is also prime and greater than $p$.
        \end{enumerate}
    \end{proof}

    \footnotetext{Os \textit{overlays} 2 e 3 só aparecem aparecem esmaecidos devido à presença do \texttt{\textbackslash setbeamercovered\{transparent\}} no documento. O comportamento padrão é a ocultação.}
\end{frame}
